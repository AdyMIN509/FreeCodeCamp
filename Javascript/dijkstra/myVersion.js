...
function shortestPath(matrix, startNode, targetNode) {
	...
	const paths = Array.from({length: 6}, (_, i) => [i]);
	...
	for (let i = 0; i < n; i++) {
		...
		for(let nodeNo = 0; nodeNo < n; nodeNo++) {
			const currentDistance = matrix[current][nodeNo];
			if(currentDistance !== INF && !visited[nodeNo]) {
				const newDistance = currentDistance + distances[current];
	...
	console.log(`Paths: ${paths[targetNode].join(" -> ")}`);

	return [paths, distances[targetNode]];
}
